/*
 * Copyright 2003-2023 The IdeaVim authors
 *
 * Use of this source code is governed by an MIT-style
 * license that can be found in the LICENSE.txt file or at
 * https://opensource.org/licenses/MIT.
 */

package com.maddyhome.idea.vim.vimscript.model.commands

import com.intellij.vim.annotations.ExCommand
import com.maddyhome.idea.vim.api.ExecutionContext
import com.maddyhome.idea.vim.api.VimEditor
import com.maddyhome.idea.vim.api.injector
import com.maddyhome.idea.vim.command.OperatorArguments
import com.maddyhome.idea.vim.ex.ranges.Range
import com.maddyhome.idea.vim.vimscript.model.ExecutionResult

@ExCommand(command = "aug[roup]")
data class AutogroupCommand(val range: Range, val modifier: CommandModifier, val argument: String) :
  Command.SingleExecution(range, modifier, argument) {

  override val argFlags: CommandHandlerFlags =
    flags(RangeFlag.RANGE_FORBIDDEN, ArgumentFlag.ARGUMENT_REQUIRED, Access.SELF_SYNCHRONIZED)

  override fun processCommand(
    editor: VimEditor,
    context: ExecutionContext,
    operatorArguments: OperatorArguments,
  ): ExecutionResult {
    val argResult = parseArgument(argument)

    if (argResult.isFailure) {
      return ExecutionResult.Error
    }

    val name = argResult.getOrThrow()

    if (name == "END") {
      injector.autoCmd.endAuGroup()
      return ExecutionResult.Success
    }

    if (modifier == CommandModifier.BANG) {
      injector.autoCmd.clearAuGroup(name)
      return ExecutionResult.Success
    }

    injector.autoCmd.startAugroup(name)
    return ExecutionResult.Success
  }

  fun parseArgument(argument: String): Result<String> {
    val split = argument.split(" ")
    if (split.size != 1) {
      return Result.failure(IllegalArgumentException("Arguments must be a single string"))
    }
    return Result.success(argument.trim())
  }
}
